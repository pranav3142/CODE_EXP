...
export default function ReportDetailsScreen() {
  ...
  return (
    <View style={styles.container}>
      <Image
        source={require('../../assets/images/manfloor.jpg')}
        style={styles.image}
      />
      <View style={styles.card}>
        <Text style={styles.alert}>{title || 'Suspicious Person Alert!'}</Text>
        <Text style={styles.subtitle}>{subtitle || 'Suspicious Loiterer'}</Text>

        <View style={styles.section}>
          <Text style={styles.sectionLabel}>ABOUT</Text>
          <Text style={styles.body}>
            {about || 'Man dressed in a black shirt with a green swirl pattern spotted loitering, laying down on the roadside while using his smartphone.'}
          </Text>
        </View>

         <View style={styles.section}>
          <Text style={styles.sectionLabel}>Time</Text>
          <Text style={styles.reporter}>{'11:20pm'}</Text>
        </View>
